...
- 파일: index.html(렌더링, 거의 수정 안 함) / data.js(업데이트 대상) / update.js(콘텐츠 변경 로그) / README.md
...
- update.js: 콘텐츠 변경(모델·제품·펀딩·인수 등)은 여기에 날짜별 기록.
...
6. update.js에 콘텐츠 변경 사항 기록 (제품·모델·펀딩·인수 등, 전문적 톤)
7. README.md 버전 히스토리 — 기술적 변경이 있을 때만 추가 (코드 구조·파일 트리·UI 로직 변경 등)
8. 변경 사항 요약을 채팅으로 보고 (커밋은 내가 직접)

### 변경 로그 이원화 원칙
- **update.js** — 일반 사용자 대상. 어떤 기업의 무엇이 왜 변경되었는지를 전문적이고 읽기 쉬운 문체로 기록. 기술적 세부사항(파일 구조, 리팩토링 등) 제외.
